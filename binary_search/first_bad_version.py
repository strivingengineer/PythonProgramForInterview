...
One Approach could be to traverse the value from 1 to n and check for each value if its bad or not,
this will be a O(n) solution. Better approach will be with the use of binary search and solve the problem in
O(log(n))
'''
...
def firstBadVersion(n):
    ...
    while low <= high:
        ...
        if isBadVersion(mid):
            ans = mid # could be a possible solution but we have to find first, so checking next possible
            high = mid - 1
        else:
            low = mid + 1
